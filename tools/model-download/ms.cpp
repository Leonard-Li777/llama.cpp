...
static std::vector<ms_file> list_files(const std::string & repo_id, const std::string & token = "") {
    ...
    std::string api_url = endpoint + "api/v1/models/" + repo_id + "/repo/files?Revision=master";
...
static std::string download_file_with_common(const ms_file & selected_file, const fs::path & local_path, const std::string & token = "", common_download_callback * callback = nullptr) {
...
static std::string resolve_file(const ms_file & file, bool offline, const std::string & token, common_download_callback * callback = nullptr) {
    ...
    return download_file_with_common(file, local_path, token, callback);
}

download_result download_model(const std::string & clean_repo_id, const std::string & filename, bool offline, const std::string & quant_tag, const std::string & token, common_download_callback * callback) {
    ...
    if (!filename.empty()) {
        for (const auto & file : all_files) {
            if (file.path == filename) {
                model_path = resolve_file(file, offline, token, callback);
                break;
            }
        }
        if (model_path.empty()) {
    ...
    for (const auto & file : all_files) {
        ...
        if (string_contains(lower, ".gguf") && string_contains(lower, "mmproj")) {
            std::string mmproj_path = resolve_file(file, offline, token, callback);
